...
{
    uint8_t step_cnt=0;
    int8_t step_num[10] = {5,2,-5,-2,2,2,-4,-4,4,20};
    for(int i=0; i<4; i++)
    {
				if(step_cnt==11)
						step_cnt=0;
...
//						delay_ms(1000);
            HAL_Delay(1000);
						stepper_move_T(SPR*step_num[step_cnt], 12000, 12000, 1000);

						ShowData(stepPosition, 12000, 12000, 1000, SPR*step_num[step_cnt]);
						step_cnt++;
				}
    }
    step_cnt=0;
